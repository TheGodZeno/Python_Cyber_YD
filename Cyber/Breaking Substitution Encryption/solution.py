import os, re, copy, pprint, pyperclip, simpleSubCipher, Testing_For_Algorithm

if not os.path.exists('allWordPatternDict.py'):
    Testing_For_Algorithm.main()  # create the wordPatterns.py file
import allWordPatternDict
...
def hackSimpleSub(message):
    ...
    for cipherword in cipherwordList:
        ...
        wordPattern = Testing_For_Algorithm.createWordPattern_v2(cipherword)
        if wordPattern not in allWordPatternDict.all_patterns:
            continue  # This word was not in our dictionary, so continue.

        # Add the letters of each candidate to the mapping.
        for candidate in allWordPatternDict.all_patterns[wordPattern]:
